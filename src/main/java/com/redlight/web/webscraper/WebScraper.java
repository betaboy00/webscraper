...
public class WebScraper
{
	...
	private static final String BMHA = "http://www.burnabyminor.com/default.aspx?p=reptryouts";
    private static final String ELEMENT_SELECTOR = "span:contains(Rep Tryout Schedule)+span";
    ...
    private static final long FREQUENCY = HALF_HOUR;
    private static String base_content = null;
		...
		public void run() {
			while (true) {
				try{
					...
					checkForChanges();
					//System.out.println(isChanged() ? "Change detected !!" : "No Change");
	...
	public static boolean isChanged() {
		...
		String current_content = getElementData();
		String difference = StringUtils.difference(base_content, current_content);
		System.out.print("Content checked on: " + Calendar.getInstance().getTime() + " .... ");
	 ...
	 * @param htmlContent The content that contain the changes
	 * @param changedStr The changes to be highlighted
	 * @return the content that contain the highlighted changes
	 */
	private static String highlightChanges(String htmlContent, String changedStr) {
		String result = null;
	...
	/**
	 * Check for changes in the content. If changes are detected, highlight the changes in the content.
	 * @return The entire content containing the highlighted changes, or null if no changes detected.
	 */
	private static String checkAndHighlightChanges() {
		String content = null;
	 ...
	 * @param content content containing the highlighted changes
	 */
	private static void sendEmail(String content) {
		...
		try {
			final NameValuePair[] data = {
					...
					new BasicNameValuePair(TO, DESTINATION),
					new BasicNameValuePair(SUBJECT, SUBJECT_STRING),
	...
	public static String getElementData() {
		String result = null;
		try {
			Document doc = Jsoup.connect(BMHA).get();
			Elements elems = doc.select(ELEMENT_SELECTOR);
	    	if (!elems.isEmpty()) {
	    		result = elems.first().toString();
	    	}
		} catch (IOException e) {
			System.out.println("Can't connect to URL: " + e);
		}
	 ...
	 * @throws IOException if not able to connect to the URL
	 */
	private static void getInitialSnapshot() throws IOException {
